...
public interface Type extends RawElement{
    ...
    public static boolean isFinite(Type type) {
        System.err.println("Unknown Type " + type.getName() + " @" + Type.class.toGenericString());
        return false;
    }
    ...
    public static Type refactor(Type rawType, Map<String, Type> typeRewriteMap, Map<String, Term> termRewriteMap) throws ValidationException {
        ...
        if (rawType instanceof InitType) {
            ((InitType) rawType).defaultValue = Term.refactor(
        ...
        if (rawType instanceof BoundedIntType) {
            ((BoundedIntType) rawType).upperBound = Term.refactor(
                    ((BoundedIntType) rawType).upperBound,
                    termRewriteMap
            );

            ((BoundedIntType) rawType).lowerBound = Term.refactor(
                    ((BoundedIntType) rawType).lowerBound,
                    termRewriteMap
            );
        }

        if (rawType instanceof TemplateType) {
            // TODO
        }
